...
| 版本 | 日期 | 变更内容 | 变更说明 |
|------|------|----------|----------|
| v1.0 | 2026-03-04 | 初始方案 | 初始重构设计方案 |
| v1.1 | 2026-03-04 | **设计变更** | 见下方变更说明 |

### v1.1 变更说明（⚠️ 重要修正）

#### 变更一：MCP工具精简

> **❌ 原方案（已废弃）**：新增3个MCP工具——`ai_analysis`、`get_analysis_progress`、`cancel_analysis`
>
> **✅ 新方案（当前）**：只保留并增强现有的 `compile_dynamic` 工具，不新增其他工具。AI分析逻辑集成到 `compile_dynamic` 的执行流程中。

#### 变更二：配置参数化设计

> **❌ 原方案（已废弃）**：`sub_agent.json` 和 `qa.json` 放在 `backend/config/` 目录，后端自动加载
>
> **✅ 新方案（当前）**：
> - 配置文件放在项目根目录的 `.aitdd/` 文件夹下（`.aitdd/sub_agent.json` 和 `.aitdd/qa.json`）
> - `compile_dynamic` 工具新增以下参数：
>   - `project_name`（string，必填）：项目名称
>   - `sub_agent_config`（object/string，必填）：AI子代理配置内容（JSON格式），**从项目根目录 `.aitdd/sub_agent.json` 文件读取内容传入**
>   - `qa_config`（object/string，必填）：问题配置内容（JSON格式），**从项目根目录 `.aitdd/qa.json` 文件读取内容传入**
> - 配置内容由MCP客户端（AI助手）读取文件后传入，后端不直接读取文件系统中的配置

#### 变更三：删除backend/config/配置文件

> **已删除**以下文件：
> - ~~`backend/config/sub_agent.json`~~
...
> **【v1.1变更】** 工具名称由 `compile_dynamic_ai` 改为使用现有的 `compile_dynamic`，配置由参数传入。
...
#### 任务4.1: 重构 compile_dynamic 工具（⚠️ v1.1变更）
- **文件**: `backend/internal/mcp/tools_other.go`
- **内容**:
  - 修改 handleCompileDynamicImpl 函数，新增 `project_name`、`sub_agent_config`、`qa_config` 三个参数
  - 从参数中解析 SubAgentConfig 和 QAConfig（不读文件系统）
  - 集成新的分析流程
  - 调用各模块完成分析

#### ~~任务4.2: 添加进度查询工具~~（❌ v1.1已废弃）
> **此任务已取消**：原方案计划新增 `get_analysis_progress` 等工具，根据v1.1变更，不再注册新工具，进度信息通过 `compile_dynamic` 的响应体直接返回。

#### 任务4.3: 更新配置管理（⚠️ v1.1变更）
- **文件**: `backend/internal/mcp/config.go`
- **内容**:
  - 添加 SubAgentConfig 和 QAConfig 的**参数解析**支持（不是文件加载）
  - 验证从参数传入的配置内容
