...
const ModuleRow: React.FC<{ moduleStats: ModuleStats }> = ({ moduleStats }) => {
  const [expanded, setExpanded] = useState(false);
  const { module, tasks, totalError, totalWarn } = moduleStats;
...
const BugLogPanel: React.FC<BugLogPanelProps> = ({
  ...
  const stats = useMemo<ModuleStats[]>(() => {
    return modules.map(module => {
      ...
      return { module, tasks: taskStats, totalError, totalWarn, totalInfo };
    });
  }, [modules, tasks]);

  // 项目级统计
  const projectError = useMemo(() => stats.reduce((s, m) => s + m.totalError, 0), [stats]);
  const projectWarn = useMemo(() => stats.reduce((s, m) => s + m.totalWarn, 0), [stats]);
